...
const EventCalendar: React.FC = () => {
	...
	const handleAddOrUpdateEvent = async () => {
		...
		formData.append('color', eventLevel);

		if (selectedEvent) {
			// Update existing event
            const response = await apiUpdateEvent(selectedEvent?.id, formData, userToken);
            if (response?.status) {
            ...
            const response = await apiCreateEvent(formData, userToken);
            if (response?.status) {
                // toastSuccess(message);
